...
export default function TabBar({ state, descriptors, navigation }) {
  ...
  const Icon = (name, focused) => {
    if (open) {
      if (isRoute(name, "home"))
        return <Images.Home width={SIZE} height={SIZE} />;
      if (isRoute(name, "ranking"))
        return <Images.Ranking width={SIZE} height={SIZE} />;
      if (isRoute(name, "store"))
        return <Images.Store width={SIZE} height={SIZE} />;
      if (isRoute(name, "mypage"))
        return <Images.Mypage width={SIZE} height={SIZE} />;
      return null;
    }

    if (isRoute(name, "home"))
      return focused ? (
        <Images.HomeSelected width={SIZE} height={SIZE} />
      ) : (
        <Images.Home width={SIZE} height={SIZE} />
      );
    if (isRoute(name, "ranking"))
      return focused ? (
        <Images.RankingSelected width={SIZE} height={SIZE} />
      ) : (
        <Images.Ranking width={SIZE} height={SIZE} />
      );
    if (isRoute(name, "store"))
      return focused ? (
        <Images.StoreSelected width={SIZE} height={SIZE} />
      ) : (
        <Images.Store width={SIZE} height={SIZE} />
      );
    if (isRoute(name, "mypage"))
      return focused ? (
        <Images.MypageSelected width={SIZE} height={SIZE} />
      ) : (
        <Images.Mypage width={SIZE} height={SIZE} />
      );
    return null;
  };
  ...
  const renderGroup = (routes) => (
    <View className="flex-row items-center">
      {routes.map((r, i) => (
        <View
          key={r?.key ?? `${i}`}
          style={{ marginRight: i < routes.length - 1 ? 40 : 0 }}
        >
          {r && renderItem(r)}
        </View>
      ))}
  ...
  return (
    <View pointerEvents="box-none" style={[StyleSheet.absoluteFillObject]}>
      {open && <BgBlack onPress={() => setOpen(false)} />}
      <View
        style={{
          position: "absolute",
          bottom: 0,
        ...
        {open && (
          ...
          >
            <TouchableOpacity
              onPress={() => {
                setOpen(false);
                router.push("/(tabs)/calculator/transport");
              }}
            ...
            <TouchableOpacity
              onPress={() => {
                setOpen(false);
                router.push("/(tabs)/calculator/diet");
              }}
              className="w-[70px] h-[70px] rounded-full border-2 border-white bg-green items-center justify-center"
